...
The next version of version control systems were distributed, meaning every developer keeps a copy of the whole repository on their computer, that are periodically synced between team members. This is what Git is.

### Why Git was created

When Linus Torvalds and other Linux kernel developers were working on the Linux kernel they used a distributed version control system called BitKeeper. The choice to use Bitkeeper was controversial amongst some Linux kernel developers, and eventually after some problems with the BitKeeper team, the Git project was started as an open source alternative that could be used for Linux development.

### How Git Works

#### Commits and File Compression
...
Essentially, much of the common content in files is extracted and put into a singular blob, and then all the blobs for a given commit are packaged together in a tree object. The tree stores the names of each file with references to the hashes of each blob. Each tree has a single hash that identifies it and these are packaged together into a commit object to show the sequential history. When a repository needs to rollback to a previous commit, git reconstructs the file by going through the tree of blobs associated with that commit and pieces them back together.
...
When you create a new branch in Git, it does not create a copy of each file and act as a separate pseudo repository (like I assumed). Rather, a branch in Git is actually essentially just a commit. Git stores a special pointer called HEAD to track the commit you are currently using.

When you create a branch, a new commit is added and HEAD is moved to track it, but from then on when you commit on that branch it creates a separate branch of commits. The most recent commits for each branch are stored in a folder at .git/refs/heads/. When you switch branches HEAD is moved to point to the corresponding branch commit. This way branches do not add large amounts of additional storage, and they can take advantage of Delta compression in the same way that normal commits do.

### How Git relates to DevOps

Modern software development often uses an Agile development cycle, where software is built piece by piece with features being added iteratively over time. Version control systems are even more important in this type of software development because the codebase will be edited in perpetuity. New features will be added rapidly, when bugs inevitably get added, Git makes it possible to quickly rollback to a previous version.
